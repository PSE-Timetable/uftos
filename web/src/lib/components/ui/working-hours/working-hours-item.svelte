<script lang="ts">
  ...
  import { WorkingHours, type TimeInterval } from '$lib/utils';
...
<div class="flex flex-row text-sm items-center gap-2 pr-2">
  <Select.Root
    selected={{ value: timeInterval.start, label: timeInterval.start }}
    onSelectedChange={(item) => (timeInterval.start = item?.value || timeInterval.start)}
    portal={null}
  >
    <Select.Trigger>
  ...
  <Select.Root
    selected={{ value: timeInterval.end, label: timeInterval.end }}
    onSelectedChange={(item) => (timeInterval.end = item?.value || timeInterval.end)}
    portal={null}
  >
    <Select.Trigger>
      <Select.Value placeholder="Uhrzeit..." />
  ...
  <div class="flex flex-row h-full gap-3">
    <button type="button" on:click={() => onItemAdd()}>
    ...
    <button type="button" on:click={() => onItemDelete()}>
