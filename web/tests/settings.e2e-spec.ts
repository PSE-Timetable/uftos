...
test.describe('Settings', () => {
  test.beforeAll('delete all existing timeslots', async ({ browser }) => {
    ...
    await expect(async () => {
      await expect(page.getByPlaceholder('Anzahl')).toBeEditable();
      await page.getByPlaceholder('Anzahl').click();
      await page.getByPlaceholder('Anzahl').fill('0');
      await expect(page.getByPlaceholder('Anzahl')).toHaveValue('0', { timeout: 250 });
      await expect(page.locator('body')).not.toContainText('1. Stunde', { timeout: 250 });
    }).toPass();
    await expect(async () => {
      await expect(page.getByPlaceholder('Länge').first()).toBeEditable();
      await page.getByPlaceholder('Länge').first().click();
      await page.getByPlaceholder('Länge').first().fill('45');
      await expect(page.getByPlaceholder('Länge').first()).toHaveValue('45');
    }).toPass();
