...
test.describe('rooms page', () => {
  test.beforeAll('delete all existing rooms', async ({ browser }) => {
    ...
    await page.locator('div:nth-child(2) > .bg-accent').nth(3).click();
    await expect(page).toHaveURL('/admin/rooms');
    await page.getByRole('row').first().waitFor();
    await page.getByRole('row', { name: 'Name Gebäude Kapazität Tags' }).getByRole('checkbox').waitFor();
    await expect(async () => {
      while (await page.locator('tbody').isVisible()) {
        await page.getByRole('row', { name: 'Name Gebäude Kapazität Tags' }).getByRole('checkbox').check();
  ...
  test('create entities', async () => {
    ...
    await expect(async () => {
      ...
      await expect(page).toHaveURL('/admin/rooms/new', { timeout: 750 });
    }).toPass();
    ...
    await expect(page.getByRole('row').nth(1)).toContainText('65');
    await page.getByRole('button', { name: 'Hinzufügen' }).click();
    await page.getByRole('textbox').first().click();
    ...
    await page.getByRole('spinbutton').click();
    await page.getByRole('spinbutton').fill('135');
    await page.getByRole('button', { name: 'Speichern' }).click();
    await page.getByRole('button', { name: 'Hinzufügen' }).click();
    await page.getByRole('textbox').first().click();
    ...
    await page.getByRole('spinbutton').click();
    await page.getByRole('spinbutton').fill('446');
    await page.getByRole('button', { name: 'Speichern' }).click();
  ...
  test('check table', async () => {
    ...
    await expect(page.getByRole('row').nth(1)).toContainText('446');
  ...
  test('edit entity', async () => {
    ...
    await expect(page.getByRole('spinbutton')).toHaveValue('446');
    await page.getByRole('textbox').first().click();
  ...
  test('select and delete', async () => {
    ...
    await page.getByRole('row', { name: 'test1 test35 135 Open menu' }).getByRole('checkbox').check();
    await expect(page.locator('body')).toContainText('1 von 3 Zeile(n) ausgewählt.');
    await page.getByRole('row', { name: 'odgsogdpnpowad pdhfgfönwßemü 65 Open menu' }).getByRole('button').click();
  ...
  test('pagination', async () => {
    for (let i = 0; i < 25; i++) {
      const requestDto: RoomRequestDto = {
        name: `Seminar${i}`,
        buildingName: `building${i}`,
        capacity: i + 35,
        tagIds: [],
      };
      await createRoom(requestDto);
    }
    await page.reload();
    ...
    await page.getByPlaceholder('Suche...').fill('Seminar1 building1');
    await expect(page.getByRole('row')).toHaveCount(2);
    await expect(page.getByLabel('Previous')).toBeDisabled();
    await expect(page.getByLabel('Next')).toBeDisabled();
    await expect(page.getByLabel('Page 1')).toHaveCSS('border-color', 'rgb(43, 109, 136)');
  ...
  test('empty input', async () => {
    ...
    await expect(page.getByText('Der Gebäudename darf nicht leer sein.')).toBeVisible();
    await expect(page.getByText('Die Kapazität darf nicht 0 sein.')).toBeVisible();
