...
public class TimefoldSolver implements SolverAdapter{
    @Override
    public Future<Timetable> solve(HashMap<PredefinedConstraints, PredefinedConstraint> definitions, List<PredefinedConstraintInstance> instances, Timetable timetable) {
        Callable<Timetable> solveTimetable = () -> {
            TimetableSolutionTimefoldInstance solution = getSolutionInstanceFromTimetableInstance(timetable);
            ...
            for (PredefinedConstraintInstance i : instances) {
                ...
                for (Resource resource : i.parameters()) {
                    switch (resource.getResourceType()) {
                        case ROOM -> {parameters.add(solution.getRooms().get(resource.getId()));}
                        case STUDENT -> {parameters.add(solution.getStudents().get(resource.getId()));}
                        case TEACHER -> {parameters.add(solution.getTeachers().get(resource.getId()));}
                        case TAG -> {parameters.add(solution.getTags().get(resource.getId()));}
                        case GRADE -> {parameters.add(solution.getGrades().get(resource.getId()));}
                        case LESSON -> {parameters.add(solution.getLessons().get(resource.getId()));}
                        case SUBJECT -> {parameters.add(solution.getSubjects().get(resource.getId()));}
                        case TIMESLOT -> {parameters.add(solution.getTimeslots().get(resource.getId()));}
                        case TIMETABLE -> {parameters.add(solution);}
    ...
    @Override
    public Future<Timetable> solve(Timetable timetable, HashMap<String, ConstraintDefinition> definitions, List<ConstraintInstance> instances) {
    ...
    TimetableSolutionTimefoldInstance getSolutionInstanceFromTimetableInstance(Timetable timetable) {
        ...
        for (StudentGroup sg : timetable.studentGroups()) {
            StudentGroupTimefoldInstance studentGroup = new StudentGroupTimefoldInstance(sg.id(), grades.get(sg.gradeId()));
        ...
        for (Lesson l : timetable.lessons()) {
            lessons.add(new LessonTimefoldInstance(l.id(), l.index(), subjects.get(l.subjectId()), studentGroups.get(l.studentGroupId())));
        }
        ...
        return new TimetableSolutionTimefoldInstance(grades, rooms, studentGroups, students, subjects, teachers, timeslots, tags, lessons);
    }
    ...
    Timetable getTimetableInstanceFromSolutionInstance(TimetableSolutionTimefoldInstance solution) {
        ...
        for (RoomTimefoldInstance r : solution.getRooms()) {
            ...
            for (LessonTimefoldInstance l : solution.getLessons().stream().filter((lesson) -> (lesson.getRoom() != null && lesson.getRoom().getId() == r.getId())).toList()) {
        ...
        for (StudentGroupTimefoldInstance sg : solution.getStudentGroups()) {
            ...
            List<Integer> lessonList = new ArrayList<>();
            for (LessonTimefoldInstance l : solution.getLessons().stream().filter((lesson) -> (lesson.getStudentGroup() != null && lesson.getStudentGroup().getId() == sg.getId())).toList()) {
                lessonList.add(l.getId());
            }
            ...
            if (sg.getGrade() == null) {
                ...
                studentGroups.add(new StudentGroup(sg.getId(), sg.getGrade().getId(), tagList, studentList, lessonList));
            }
        }
        ...
        for (SubjectTimefoldInstance s : solution.getSubjects()) {
            ...
            List<Integer> lessonList = new ArrayList<>();
            for (LessonTimefoldInstance l : solution.getLessons().stream().filter((lesson) -> (lesson.getSubject() != null && lesson.getSubject().getId() == s.getId())).toList()) {
                lessonList.add(l.getId());
            }
        ...
        for (TeacherTimefoldInstance t : solution.getTeachers()) {
            ...
            List<Integer> lessonList = new ArrayList<>();
            for (LessonTimefoldInstance l : solution.getLessons().stream().filter((lesson) -> (lesson.getTeacher() != null && lesson.getTeacher().getId() == t.getId())).toList()) {
                lessonList.add(l.getId());
            }
        ...
        for (TimeslotTimefoldInstance t : solution.getTimeslots()) {
            ...
            List<Integer> lessonList = new ArrayList<>();
            for (LessonTimefoldInstance l : solution.getLessons().stream().filter((lesson) -> (lesson.getTimeslot() != null && lesson.getTimeslot().getId() == t.getId())).toList()) {
                lessonList.add(l.getId());
            }
        ...
        for (TagTimefoldInstance t : solution.getTags()) {
            ...
            tags.add(new Tag(t.getId(),studentList, studentGroupList, teacherList, roomList, subjectList, gradeList, timeslotList));
        }
        ...
        for (LessonTimefoldInstance l : solution.getLessons()) {
            ...
            lessons.add(new Lesson(l.getId(), l.getIndex(), timeslotId, teacherId, studentGroupId, subjectId, roomId));
        }

        return new Timetable(grades, rooms, studentGroups, students, subjects, teachers, timeslots, lessons, tags);
